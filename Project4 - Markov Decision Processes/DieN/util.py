...
def build_transition_matrix(num_states, N, B):
    ...
    Dnum_states = num_states * 2
    roll_t = np.empty((0, Dnum_states))

    for s in range(Dnum_states):
        state_row = np.zeros(Dnum_states)
        for s_prime in range(1, Dnum_states):
            ...
            if s == s_prime >= num_states:
                state_row[s_prime] = 1.  # terminal states remain same
            elif s < s_prime <= s + N < num_states:
    ...
    for i in range(Dnum_states):
        if sum(roll_t[i]) < 1:
            roll_t[i, num_states] = 1. - sum(roll_t[i])  # 1. * sum(B) / N
    ...
    quit_t = np.zeros((Dnum_states, Dnum_states))
    for i in range(Dnum_states):
        quit_t[i][i % (num_states) + num_states] = 1.
