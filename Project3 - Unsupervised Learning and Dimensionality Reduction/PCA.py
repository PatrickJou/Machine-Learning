...
# Run PCA
pca = PCA(random_state=5).fit(X1) #for all components
...
plt.title("PCA Explained Variance and Eigenvalues: " + Dataset)
fig.tight_layout()
plt.show()

###################################################################
# PCA Dimensional Reduction
X1 = PCA(n_components=5,random_state=5).fit_transform(X1)
###################################################################
...
plt.title("Cluster Score for K Mean (KM) on PCA " + Dataset)
...
plt.title("Fitness Time for K Mean (KM) on PCA " + Dataset)
plt.xlabel("K cluster")
...
plt.title("Cluster Score for Expectation Maximization (EM) on PCA " + Dataset)
fig.tight_layout()
plt.show()


# [EM] Plot the Fitness Time over K cluster
plt.title("Fitness Time for Expectation Maximization (EM) on PCA " + Dataset)
plt.xlabel("K cluster")
plt.ylabel("Fitness Time (s)")
#plt.ylim(0.0, 1.1)
lw = 2
